...
MmscSettings *mms_load_mmsc_settings(Octstr *fname, List **proxyrelays, int skip_admin)
{
     ...
     if ((m->mm5 = _mms_load_module(cfg, grp, "mm5-module", "mm5_funcs", shell_mm5)) != NULL) {
	  Octstr *s = mms_cfg_get(cfg, grp, octstr_imm("mm5-module-parameters"));
     ...
     if (mms_cfg_get_bool(cfg, grp, octstr_imm("optimize-notification-size"), 
			  &m->optimize_notification_size) < 0)
	  m->optimize_notification_size = 1;
