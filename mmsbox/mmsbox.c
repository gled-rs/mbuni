...
#else
     m  = make_msg_from_sendmms_request(from, xto, e ? e->msgId : NULL,
					subject, data, ctype, reply_headers, msg_url,
					base_url, type, 
					mmc, svc_name, expiryt, 
					mt_multipart, 
					rr_url != NULL,
					dlr_url != NULL,
					allow_adaptations,
...
static void dispatch_sendmms_recv(List *rl)
{
     ...
     while ((h = gwlist_consume(rl)) != NULL) {
	  ...
	  parse_cgivars(h->headers, h->body, &h->cgivars, &cgivar_ctypes);
	  ...
	  if ((u = auth_user(username, password)) != NULL && 
	      is_allowed_ip(sendmms_port.allow_ip, sendmms_port.deny_ip, h->ip)) {
...
#else
	       get_content_from_sendmms_request(h->body, h->headers, h->cgivars, 
						cgivar_ctypes, 
	       ...
	       if ((xs = http_header_value(rh, octstr_imm("Content-Type"))) == NULL && ctype)
	       ...
	       octstr_destroy(data); /* get_content_from_sendmms now gives us a copy of it */
	  } else {
